...
[![Live App](https://img.shields.io/badge/🌐_Live_App-agriguardianproject1.lovable.app-2ea44f?style=for-the-badge)](https://agriguardianproject1.lovable.app/)
[![License](https://img.shields.io/badge/License-MIT-blue?style=for-the-badge)](#-license)
[![Made with TanStack Start](https://img.shields.io/badge/Frontend-TanStack_Start-FF4154?style=for-the-badge)](https://tanstack.com/start)
[![Python](https://img.shields.io/badge/Backend-Python-3776AB?style=for-the-badge&logo=python&logoColor=white)](#-backend-setup--cross-platform-ocr)

> 📽️ **[Watch Demo Video](#)**  •  🌐 **[Try the Live App](https://agriguardianproject1.lovable.app/)**  •  📂 **[Source Code](https://github.com/Bui9ldWithShaurya/AgriGuardian_Project)**

---

## 📖 Table of Contents

- [About AgriGuardian](#-about-agriguardian)
...
- [Backend Setup & Cross-Platform OCR](#-backend-setup--cross-platform-ocr)
- [🖥️ Frontend Setup](#️-frontend-setup)
- [Environment Variables](#-environment-variables)
- [Production Deployment Blueprints](#-production-deployment-blueprints)
...
1. **Detects crop disease instantly** using an on-device CNN (EfficientNetB3, exported to ONNX) that runs entirely **in the browser** — no cloud round-trip, works even in low-connectivity rural areas.
2. **Digitizes printed farm documents** — soil health cards, fertilizer/pesticide labels, and agronomic reports — via an **OCR pipeline** on the backend, turning them into structured, searchable data.
3. **Fuses sensor + vision data** into a single risk score and generates **plain-language, bilingual (English/Hindi) advisory** with concrete treatment steps.
4. **Alerts farmers proactively** via SMS/notifications when conditions turn risky, and visualizes trends on a dashboard built for low-literacy, mobile-first use.
...
> The layout above reflects the full-stack project. This repository's `frontend/` corresponds to the web app in this ZIP export; see [`backend/`](#-backend-setup--cross-platform-ocr) for the Python service.
...
| Tool | Version | Notes |
|---|---|---|
| [Bun](https://bun.sh) | ≥ 1.1 | Frontend package manager & dev server (Node.js ≥ 18 also works via `npm`) |
| [Python](https://www.python.org/) | ≥ 3.10 | Backend runtime |
| [Tesseract OCR](https://github.com/tesseract-ocr/tesseract) | ≥ 5.0 | Native OCR engine binary (see below) |
...
> 💡 If `pytesseract` can't find the binary automatically, set the path explicitly in your `.env` (see [Environment Variables](#-environment-variables)) or point to it directly in code via `pytesseract.pytesseract.tesseract_cmd`.
...
**Steps:**
1. Push `render.yaml` to your repo root (or `backend/`).
2. In the [Render Dashboard](https://dashboard.render.com/), select **New → Blueprint** and connect this repository.
3. Render provisions the web service automatically, installing the native Tesseract binary via `buildCommand`.
4. Add secret env vars (`WEATHER_API_KEY`, `SMS_API_KEY`) in the Render dashboard.
5. Note the deployed URL (e.g. `https://agriguardian-backend.onrender.com`) — you'll need it for the frontend.
...
> Our current live demo is hosted at **[agriguardianproject1.lovable.app](https://agriguardianproject1.lovable.app/)**.
...
<p align="center">
  <b>Built with passion. Deployed for impact. AI for a healthier farming future. 🌾</b><br/>
  <sub>Smart insights for a better tomorrow — Team SPARK</sub>
</p>
